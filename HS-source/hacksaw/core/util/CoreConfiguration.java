...
public class CoreConfiguration extends Configuration
{
	...
	public static void init(File minecraftDir, String path)
	{
		if (INSTANCE != null)
		{
			HacksawLogger.log(LogLevel.WARNING,
					"Blocked attempt to Re-Init configuration");
			return;
		}
		final File file;
	...
	private void populateDefaults()
	{
		...
		// Gets the True/False for whether Vanilla recipes should or shouldn't
		// be used for cooking
		Property prop = get(Configuration.CATEGORY_GENERAL,
				USE_VANILLA_RECIPES, false);
		...
		prop = get(Configuration.CATEGORY_ITEM, "food.carrot.sliced",
				nextItemId());
		...
		prop = get(Configuration.CATEGORY_ITEM, "food.lettuce.sliced",
				nextItemId());
		...
		prop = get(Configuration.CATEGORY_ITEM, "food.lamb.chop.raw",
				nextItemId());
		...
		prop = get(Configuration.CATEGORY_ITEM, "food.lamb.chop.cooked",
				nextItemId());
		...
		prop = get(Configuration.CATEGORY_ITEM, "food.carrot.seed",
				nextItemId());
		HacksawItems.carrotSeed.itemId = prop.getInt();
		HacksawItems.carrotSeed.name = "Carrot Seed";

		prop = get(Configuration.CATEGORY_ITEM, "food.lettuce.seed",
				nextItemId());
		...
		prop = get(Configuration.CATEGORY_ITEM, "chef.knife.sharp",
				nextItemId());
		...
		prop = get(Configuration.CATEGORY_ITEM, "chef.knife.sharpener",
				nextItemId());
	...
	private int nextBlockId(int id)
	{
		...
		while (Block.blocksList[id] != null
				|| claimed_blocks.contains(Integer.valueOf(id)))
		{
			++id;
		}
		if (oid != id)
		{
			HacksawLogger.log("Block ID conflict @ " + oid + ", taken by "
					+ Block.blocksList[oid] + ", assigning " + id);
		}
	...
	private int nextItemId(int id)
	{
		if (!autoresolve)
			return id;
		final int oid = id;
		while (Item.itemsList[id + 256] != null
				|| claimed_items.contains(Integer.valueOf(id)))
		{
			++id;
		}
		if (oid != id)
		{
			HacksawLogger.log("Item ID conflict @ " + oid + ", taken by "
					+ Item.itemsList[oid + 256] + ", assigning " + id);
		}
		// TODO :: Resolved Null pointer on Add.
